...
With **`provider: "brevo"`** in `js/enquiry-config.js`, the **`enquiry-brevo`** server function sends email via Brevo, then **POSTs** to your Apps Script web app so **`EnquiryWebhook.gs`** can append a row, and **upserts a Lead in Zoho CRM** when `ZOHO_*` env vars are set (`docs/zoho/README.md`). By default it uses the same URL as **`webAppUrl`** in `js/enquiry-config.js`; set **`APPS_SCRIPT_ENQUIRY_URL`** in hosting env only to override. The function uses **`redirect: follow`** (Node keeps the POST through Google’s 302 to `googleusercontent.com`). The payload **`secret`** sent to Apps Script is taken from **`APPS_SCRIPT_ENQUIRY_SECRET`** or **`BREVO_ENQUIRY_SECRET`** when set, so it matches **Script properties → `ENQUIRY_SECRET`**. **Redeploy** the Apps Script **Web app** after changing `EnquiryWebhook.gs`. A successful API response may include **`sheetLogged: true`**; if the sheet step fails, check **`sheetError`** / **`sheetHint`** in the JSON. Restart **`netlify dev`** after editing `.env`.
