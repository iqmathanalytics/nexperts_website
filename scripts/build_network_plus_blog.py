"""Build Network+ blog from docx - one-off generator."""
...
SLUG = "comptia-network-plus-certification-guide"
IMG_SLUG = "comptia-network-plus-certification-guide-for-beginners"
META_TITLE = "CompTIA Network+ Certification Guide for Beginners"
META_DESC = (
    "Learn how to pass the CompTIA Network+ N10-009 exam, explore costs, salary prospects, "
    "study tips, and career opportunities for beginners."
)
DOCX = ROOT / "Complete CompTIA Network+ Certification Guide for Beginners.docx"
...
def _table_html(tbl: ET.Element) -> str:
    ...
    if len(rows) >= 2:
        ...
        if all(head) and len(head) == ncols:
            thead = "<thead><tr>" + "".join(
                f'<th scope="col">{c}</th>' for c in head
            ) + "</tr></thead>"
            tbody_rows = rows[1:]
    body = ""
...
def docx_to_body(docx_path: Path) -> tuple[str, list[tuple[str, str]], str]:
            ...
            for blip in drawing.iter(f"{A}blip"):
                ...
                if rid and rid in rel_map:
                    ...
                    blocks.append(
                        ...
                        f'alt="CompTIA Network+ certification guide illustration" loading="lazy" width="720" height="405"></figure>'
                    )
                    break
    ...
    flush_list()
    body = "\n        ".join(blocks)
    body = re.sub(r"(<h[23]>)(<strong>[^<]*</strong>)+", lambda m: m.group(1) + re.sub(r"</?strong>", "", m.group(0)[len(m.group(1)):]), body)
    body = re.sub(
        r"(<h[23]>)(.*?)(</h[23]>)",
    ...
    if not title:
        title = "Complete CompTIA Network+ Certification Guide for Beginners"
...
def build_page() -> None:
    ...
    body = body.replace(
        "The CompTIA Network+ certification is a globally recognized",
        'The <a href="https://www.nexpertsacademy.com/courses/comptia-network-plus">CompTIA Network+ certification</a> is a globally recognized',
        1,
    )
    body = body.replace(
...
{nav}
<section class="hero">
  <p style="font-size:.72rem;color:var(--ink3);margin-bottom:14px"><a href="/" style="color:inherit;text-decoration:none">Home</a> / <a href="/blog" style="color:inherit;text-decoration:none">Blog</a> / CompTIA Network+ Guide</p>
  <h1>CompTIA Network+ Certification Guide<br><em>for beginners</em></h1>
  <p>May 2026 · Nexperts CompTIA Practice</p>
</section>
...
{body_html}
    <div class="card-grid">
      <a class="card" href="/courses/comptia-network-plus">
        <h3>CompTIA Network+ Training</h3>
        <p>Official N10-009 exam prep programme.</p>
        <span class="card-link">Learn more →</span>
      </a>
      <a class="card" href="/ccna">
        <h3>CCNA Training</h3>
        <p>Compare with Cisco CCNA path.</p>
        <span class="card-link">Learn more →</span>
      </a>
  ...
  <p style="margin-top:2rem"><a href="/contact-us#enquire" style="color:var(--blue);font-weight:700">Enquire about Network+ training →</a></p>
</section>
<footer>
