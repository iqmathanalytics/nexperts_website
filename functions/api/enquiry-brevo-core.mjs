  ...
  if (!apiKey || !senderEmail) {
    return {
      ...
      body: JSON.stringify({
        ...
        hint: "Set BREVO_API_KEY and BREVO_SENDER_EMAIL in the hosting environment (Cloudflare Pages / Netlify).",
      }),
    };
  }
  ...
  if (
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        error: "forbidden",
        hint:
          "BREVO_ENQUIRY_SECRET is set on the server but the form did not send the same secret. Remove BREVO_ENQUIRY_SECRET in hosting env, or set the same value as secret in js/enquiry-config.js and redeploy.",
      }),
    };
  }
